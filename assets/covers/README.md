# Cover artwork — slice-packer-torus

![A square grid of graphite accelerator tiles with a contiguous slate-blue allocation and return connectors.](social-preview-v1.jpg)
...
Open [repository settings](https://github.com/dimaggi-ai/slice-packer-torus/settings),
...
Alt text: A square grid of graphite accelerator tiles with a contiguous slate-blue allocation and return connectors.
...
Subject: rectangular allocation within a torus-connected accelerator pod. One simple flat square graphite allocation board with a neat 5 by 5 grid of shallow square tiles. Exactly one contiguous 2 by 3 rectangle is colored muted slate blue; the other tiles are charcoal. A few plain pale-metal U-shaped return connectors visibly join opposite board edges, suggesting wraparound topology. The board is one coherent sculptural object on the chalk tabletop, not a sprawling chip city. Keep connectors sparse and geometry crisp. No screens, dimensions, labels, Tetris shapes, cables everywhere or floating components.
```
